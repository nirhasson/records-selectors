...
async function processSpotifyMatch(album) {
  ...
  if (!spotifyAlbum && !spotifyTrack && !exactArtistResult) {
    ...
    if (artistAlbums && artistAlbums.albums.length > 0) {
      ...
      artistAlbumMatches.slice(0, 3).forEach((match, i) => {
        console.log(
          `${i + 1}. "${match.album.name}" (Score: ${match.score.toFixed(2)}, Title: ${match.titleScore.toFixed(2)}, Year: ${match.yearScore.  (Score: ${match.score.toFixed(2)}, Title: ${match.titleScore.toFixed(2)}, Year: ${match.yearScore.toFixed(2)})`,
        )
      })
